		...
		if (budy.includes("Gracias")) {
			...
			if (budy.includes("Bot")) {
				...
				if (budy.includes("Kuin")) {
					...
					if (budy.includes("Jacksito")) {
						if (!isGroup) return
						reply('Esta durmiendo que quieres?')
						}
					if (budy.includes('foto')){
						Dhani.updatePresence(from, Presence.composing)
						rm = [
						result = fs.readFileSync(`./database/image/dhani.jpg`),
						]
						nk = rm[Math.floor(Math.random() * rm.length)]
						Dhani.sendMessage(from, nk, image, {quoted: mek})
						}
...
Dhani.sendMessage (from, fx, MessageType.text, {quoted: mek, "forwardingScore": 9999, "isForwarded": true})
break
