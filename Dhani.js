...
if (args[0] === 'on') {
if (isWelkom) return reply('Esta activo gilaso')
_welkom.push(from)
fs.writeFileSync('./database/welcome.json', JSON.stringify(_welkom))
reply(`\`\`\`Listo ✅, Vamos a darle la auto bienvenida a estos venecos\`\`\` *${groupMetadata.subject}*`)
} else if (args[0] === 'off') {
if (!isWelkom) return reply('esta muerto mi loco')
var ini = _welkom.indexOf(from)
_welkom.splice(ini, 1)
fs.writeFileSync('./database/welcome.json', JSON.stringify(_welkom))
reply(`\`\`\`Listo ✅, Desactivaste la bienvenida\`\`\` *${groupMetadata.subject}*`)
} else if (!c){
anu =`Por favor elige uno\nPara la función de bienvenida`
...
if (!isGroupAdmins && !mek.key.fromMe) return reply(mess.only.admin)
if (args[0] === 'on') {
if (isAntiLink) return reply('Ya esta activo gilaso')
_antilink.push(from)
fs.writeFileSync('./database/antilink.json', JSON.stringify(_antilink))
reply(`\`\`\`listo ✅, Ya no pueden venir giles a hacer spam\`\`\` *${groupMetadata.subject}*`)
} else if (args[0] === 'off') {
if (!isAntiLink) return reply('esta muerto loco')
var ini = _antilink.indexOf(from)
_antilink.splice(ini, 1)
fs.writeFileSync('./database/antilink.json', JSON.stringify(_antilink))
reply(`\`\`\`Listo ✅, Desactivaste el antilink Gay\`\`\` *${groupMetadata.subject}*`)
} else if (!c){
anu =`Por favor elige uno\nPara función antienlace`
