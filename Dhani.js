...
case '8bit':
if (args.length < 1) return reply(`*Ejemplo : ${prefix + command} nombre1 y nombre2*`)
var F = body.slice(5)
var F1 = F.split("&")[0];
var F2 = F.split("&")[1]; 
reply(mess.wait)
...
Dhani.sendMessage(from, pornhub, image, {caption: `*By: JᴀᴄᴋSɪᴛᴏ-⃫⃟冰⁩他妈的*`, quoted: mek})
break
case 'pornhub':
if (args.length < 1) return reply(`*Ejemplo : ${prefix + command} nombre1 y nombre2*`)
var F = body.slice(8)
...
Dhani.sendMessage(from, girlneko, image, {caption: `*By: JᴀᴄᴋSɪᴛᴏ-⃫⃟冰⁩他妈的*`, quoted: mek})
break
case 'sadboy':
if (args.length < 1) return reply(`*Ejemplo : ${prefix + command} nombre1 y nombre2*`)
var F = body.slice(7)
...
cowok = `aquí está el chico`
cewek = `Aquí está la chica`
text2 = `Haga clic en siguiente para ir a${command} próximo`
